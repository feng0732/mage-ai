...
[variable_type != SPARK_DATAFRAME] ?
    ↓ YES           ↓ NO (Spark 跳过
write_metadata()  → 写 metadata.json (type/types)
    ↓
__write_resource_usage()  → 写 resource_usage.json
```
...
1. **同步写入** — variable.py 第 795-798 行
```python
...
2. **异步写入** — variable.py 第 885-887 行
```python
if self.variable_type != VariableType.SPARK_DATAFRAME:
    self.write_metadata()
```
...
**写入边界：**
- ✅ **所有类型 **EXCEPT** SPARK_DATAFRAME** 才跳过（避免 Spark 目录中出现 json 导致读错误
...
1. **__write_resource_usage() — variable.py 第 948-951 行在 `__write_data` 和 `__write_data_async` 的末尾统一调用：
```python
# 在 write_metadata() 之后（或 Spark 跳过之后
...
| 复杂对象异步 | 第 684-687 行 | 同上 |
| 自定义 joblib 对象 | 第 696-699 行 | 同上 |
| DataManager 写入后 | 同步：第 748-751 行 | directory, size（从 data_manager.resource_usage 复制 |
| DataManager 写入后异步 | 第 844-847 行 | 同上 |
| __write_json 同步 | 第 1072-1075 行 | size, path |
| __write_json_async | 第 1086-1089 行 | 同上 |
...
| 异步 JSON | aiofiles | （TODO 同步实现） | （TODO 同步实现） |
...
阅读起点：从 BlockExecutor.execute() → \_execute() → block.execute_sync()

### 9.1 变量存储入口阅读顺序

1. `Block.store_variables()` — models/block/__init__.py L3776-3861
2. `VariableManager.add_variable()` — variable_manager.py L61-162
3. `Variable.write_data()` — models/variable.py L720-812
4. 分派看 `__write_data()` — 同上 L727-812
5. 最后 `write_metadata()` + `__write_resource_usage()`

### 9.2 变量读取阅读顺序

1. 上游取数起点：`fetch_input_variables()` / `get_outputs()
2. `VariableManager.get_variable()` → variable_manager.py L326-365
3. `Variable.read_data()` → models/variable.py L385-419
4. 看 `__read_data()` 类型分派读

### 9.3 动态块阅读顺序

1. `uuid_for_output_variables() — dynamic/utils.py L160起 → 看 block_uuid 拼接
2. `get_variable_objects() — dynamic/variables.py L307起
