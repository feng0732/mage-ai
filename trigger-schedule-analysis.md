...
| 类/枚举 | 起始行 | 说明 |
|-----------|--------|------|
| `ScheduleStatus` | [L22](./mage_ai/data_preparation/models/triggers/__init__.py#L22-L25) | 状态枚举：`ACTIVE` / `INACTIVE` |
| `ScheduleType` | [L27](./mage_ai/data_preparation/models/triggers/__init__.py#L27-L31) | 类型枚举：`TIME` / `EVENT` / `API` |
| `ScheduleInterval` | [L40](./mage_ai/data_preparation/models/triggers/__init__.py#L40-L47) | 间隔枚举：`@once` / `@hourly` / `@daily` / `@weekly` / `@monthly` / `@always_on` |
| `SettingsConfig` | [L50](./mage_ai/data_preparation/models/triggers/__init__.py#L50-L58) | 高级设置（并发控制、landing time 等 |
| `Trigger` | [L61](./mage_ai/data_preparation/models/triggers/__init__.py#L61-L117) | Trigger 配置主类 |
...
| 函数 | 起始行 | 说明 |
|------|--------|------|
| `get_triggers_by_pipeline()` | [L232](./mage_ai/data_preparation/models/triggers/__init__.py#L232-L238) | 获取管道的所有触发器 |
| `load_trigger_configs() | [L285](./mage_ai/data_preparation/models/triggers/__init__.py#L285-L300) | 从 yaml 内容加载触发器配置 |
| `add_or_update_trigger_for_pipeline_and_persist() | [L303](./mage_ai/data_preparation/models/triggers/__init__.py#L303-L329) | 添加/更新触发器并持久化到文件 |
...
**类起始行**：[L86](./mage_ai/orchestration/db/models/schedules.py#L86-L766)

**核心方法**：

| 方法 | 起始行 | 说明 |
|------|--------|------|
| `active_schedules()` | [L293](./mage_ai/orchestration/db/models/schedules.py#L293-L299) | 类方法，获取所有活跃调度 |
| `create()` | [L302](./mage_ai/orchestration/db/models/schedules.py#L302-L306) | 类方法，创建调度 |
| `create_or_update_batch()` | [L309](./mage_ai/orchestration/db/models/schedules.py#L309-L407) | 批量创建/更新（sync_schedules 调用 |
| `current_execution_date()` | [L438](./mage_ai/orchestration/db/models/schedules.py#L438-L508) | 计算当前执行日期 |
| `next_execution_date()` | [L510](./mage_ai/orchestration/db/models/schedules.py#L510-L534) | 计算下次执行日期 |
| `should_schedule()` | [L537](./mage_ai/orchestration/db/models/schedules.py#L537-L679) | **核心决策方法**，判断是否应该调度新运行 |
| `landing_time_enabled()` | [L681](./mage_ai/orchestration/db/models/schedules.py#L681-L693) | 是否启用落地时间 |
| `runtime_history()` | [L725](./mage_ai/orchestration/db/models/schedules.py#L725-L750) | 获取历史运行时长 |
...
**类起始行**：[L768](./mage_ai/orchestration/db/models/schedules.py#L768-L1388)

**状态枚举 `PipelineRunStatus`：[L769-L774)
- `INITIAL`：初始状态，已创建未开始
- `RUNNING`：运行中
- `COMPLETED`：已完成
- `FAILED`：失败
...
| 方法 | 起始行 | 说明 |
|------|--------|------|
| `create()` | [L1353](./mage_ai/orchestration/db/models/schedules.py#L1353-L1388) | 创建运行，含 `prevent_duplicates` 去重参数 |
| `executable_block_runs()` | [L978](./mage_ai/orchestration/db/models/schedules.py#L978-L1221) | 获取当前可执行的 Block 列表 |
| `update_block_run_statuses()` | [L1223](./mage_ai/orchestration/db/models/schedules.py#L1223-L1301) | 级联更新 Block 运行状态 |
...
**类起始行**：[L1663](./mage_ai/orchestration/db/models/schedules.py#L1663-L1922)

**状态枚举 `BlockRunStatus`：[L1664-L1672)
...
**类起始行**：[L1924](./mage_ai/orchestration/db/models/schedules.py#L1924-L2023)

用于事件触发模式，通过 JSON 模式递归匹配事件内容。

| 方法 | 起始行 | 说明 |
|------|--------|------|
| `match()` | [L2007](./mage_ai/orchestration/db/models/schedules.py#L2007-L2022) | 递归匹配事件字典与 pattern |
| `active_event_matchers() | [L1942](./mage_ai/orchestration/db/models/schedules.py#L1942-L1945) | 获取所有活跃的事件匹配器 |
...
**处理类**：`ApiTriggerPipelineHandler`，`post()` 方法 [L18](./mage_ai/server/api/triggers.py#L18-L65)
...
**处理类**：`ApiRunHandler`，`post()` 方法 [L27](./mage_ai/server/api/runs.py#L27-L150)
...
**函数起始行**：[L83](./mage_ai/orchestration/triggers/utils.py#L83-L136)
...
**函数起始行**：[L1656](./mage_ai/orchestration/pipeline_scheduler_original.py#L1656-L1955)
...
| 方法 | 起始行 | 说明 |
|------|--------|------|
| `start()` | [L131](./mage_ai/orchestration/pipeline_scheduler_original.py#L131-L203) | 启动管道运行 |
| `stop()` | [L206](./mage_ai/orchestration/pipeline_scheduler_original.py#L206-L210) | 停止管道运行 |
| `schedule()` | [L213](./mage_ai/orchestration/pipeline_scheduler_original.py#L213) | 调度可执行 Block |
| `on_block_complete()` | [L377](./mage_ai/orchestration/pipeline_scheduler_original.py#L377-L410) | Block 完成回调 |
| `on_block_failure()` | [L443](./mage_ai/orchestration/pipeline_scheduler_original.py#L443-L489) | Block 失败回调 |
...
**函数起始行**：[L2067](./mage_ai/orchestration/pipeline_scheduler_original.py#L2067-L2108)
...
**函数起始行**：[L2110](./mage_ai/orchestration/pipeline_scheduler_original.py#L2110-L2129)
...
**函数起始行**：[L1606](./mage_ai/orchestration/pipeline_scheduler_original.py#L1606-L1654)
